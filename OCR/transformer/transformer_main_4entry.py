...
def model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope("model"):
        ...
        conv = cnn_layer.CNNNetwork(4, 2, [5, 5], 0.4, mode == tf.estimator.ModeKeys.TRAIN)
        model = transformer.Transformer(params, mode == tf.estimator.ModeKeys.TRAIN)

        conv_output = conv(inputs)
        log = [model(conv_output[0], targets["line1"]),
        ...
        for line, logits in enumerate(log):
            ...
            if mode == tf.estimator.ModeKeys.EVAL:
                ...
                return tf.estimator.EstimatorSpec(
                    ...
                    eval_metric_ops=metrics.get_eval_metrics(logits, list(labels.keys())[line], params))
            else:
